...
img_height = 300
doom_host = "127.0.0.1"
trained_model = "gestures_14_11924-11586_arch5.model"
num_classes = 11
...
if __name__ == "__main__":
    ...
    if cap.isOpened():
        while True:
            ...
            if index == 0 and lastIndex == 0 and score > 0:
                requests.get("http://{}/backward".format(doom_host))
            elif index == 1 and lastIndex == 1 and score > 0:
                requests.get("http://{}/crouch".format(doom_host))
            elif index == 2 and lastIndex == 2 and score > 0:
                requests.get("http://{}/forward".format(doom_host))
            elif index == 3 and lastIndex == 3 and score > 0:
                requests.get("http://{}/god_mode".format(doom_host))
            elif index == 4 and lastIndex == 4 and score > 0:
                requests.get("http://{}/jump".format(doom_host))
            elif index == 5 and lastIndex == 5 and score > 0:
                requests.get("http://{}/left".format(doom_host))
            elif index == 6 and lastIndex == 6 and score > 0:
                requests.get("http://{}/next_weapon".format(doom_host))
            elif index == 7:
                pass
            elif index == 8 and lastIndex == 8 and score > 0:
                requests.get("http://{}/right".format(doom_host))
            elif index == 9 and lastIndex == 9 and score > 0:
                requests.get("http://{}/fire".format(doom_host))
            elif index == 10 and lastIndex == 10 and score > 0:
